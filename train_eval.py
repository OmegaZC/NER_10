...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    if config.model_name == 'Transformer':
        ...
        # len(train_iter) * config.num_epochs:总步数
        # scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(optimizer,
        #     num_warmup_steps=int(config.warmup_proportion * len(train_iter) * config.num_epochs),
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(pbar):
            ...
            if total_batch % 100 == 0:
                ...
                msg = '\nIter: {0:>6},  Train Loss: {1:>5.4},  Train Acc: {2:>6.4%},  Val Loss: {3:>5.4},  Val Acc: {4:>6.4%},  Time: {5} {6}'
